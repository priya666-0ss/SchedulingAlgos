...
bool check_val(vector<bool> check){
...
void lottery_scheduling(vector<process> processes){
    ...
    vector<int> tat;
    vector<int> wat;
    int time = processes[0].get_arrival() + processes[0].get_burst();

    
    ct[0] = time;
    
   vector<bool> check(processes.size(), false);
   //cout << check.size() << endl;
   check[0] = true;
   while(check_val(check) != true){
       int ranIndex = rand() % processes.size();
       
       if(check[ranIndex] == false && time >= processes[ranIndex].get_arrival()){
           time = time + processes[ranIndex].get_burst();
   ...
   //cout << endl;
    ...
    for(int i = 0; i < processes.size(); i++){
        int val1 = ct[i] - processes[i].get_arrival();
        tat.push_back(val1);
        avg1 = avg1 + val1;
        int val2 = tat[i] - processes[i].get_burst();
        wat.push_back(val2);
        avg2 = avg2 + val2;
    }

    avg1 = float((avg1) / processes.size());
    avg2 = float((avg2) / processes.size());

    cout << "ALgorithm- lottery-scheduling: " << endl;
    cout << "Turn Around Time <min average max> ";

    cout << *max_element(tat.begin(), tat.end()) << " ";
    cout << avg1 << " ";
    cout << *min_element(wat.begin(), wat.end()) << " " << endl;

    cout << "Response Time <min average max> ";
    cout << *max_element(wat.begin(), wat.end()) << " ";
    cout << avg2 << " ";
    cout << *min_element(wat.begin(), wat.end()) << " " << endl;

    cout << "wait Around Time <min average max> ";
    cout << *max_element(wat.begin(), wat.end()) << " ";
    cout << avg2 << " ";
    cout << *min_element(wat.begin(), wat.end()) << " " << endl;

    cout << avg1 << " " << avg2 << " " << avg2 << endl;
